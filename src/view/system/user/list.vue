...
let { List, filteredList } = storeToRefs(appStore)
appStore.getList()
...
const selectedIds = ref<string[]>([]); // 用于存储选中的学生id
...
// 查询学生信息
const onSearch = () => {
    const results = appStore.search(searchForm.value);
...
const handleSelectionChange = (val) => {
    selectedIds.value = val.map((student) => student.id);
}
// 批量删除
const BatchDelete = () => {
    ...
    appStore.deletes(selectedIds.value);
    ElMessage.success('学生信息删除成功');
    selectedIds.value = []; // 清空选中的学生id
    getByPage(); // 重新获取学生列表
}
...
function showDialog(id: string) {
    const student = List.value.filter(item => {
        return item.id === id
    })
    console.log(student);
    form.id = id
